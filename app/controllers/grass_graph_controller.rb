...
class GrassGraphController < ActionController::API
  ...
  def service
    tmpfile_path = "./tmp/gg_others_svg/#{params['github_id']}_#{Time.now.strftime('%Y-%m-%d')}.svg"
    unless ( File.exists?(tmpfile_path) && File.size(tmpfile_path) != 0)
      `curl https://github.com/#{params['github_id']} | awk '/<svg.+class="js-calendar-graph-svg"/,/svg>/' | \
      sed -e 's@<svg@<svg xmlns="http://www.w3.org/2000/svg"@' > #{tmpfile_path}`
    end
